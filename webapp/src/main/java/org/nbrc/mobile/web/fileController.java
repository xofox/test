...
import org.nbrc.mobile.helper.NameComparator;
import org.nbrc.mobile.helper.SizeComparator;
import org.nbrc.mobile.helper.TypeComparator;
...
public class fileController extends BaseController {
	...
	public Map<String, Object> fileManage(HttpServletResponse response, HttpServletRequest request, HttpSession session) {
		...
		String rootPath = session.getServletContext().getRealPath("/") + ConfigUtil.get("fileSavePath") +"/";

		// 根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/attached/
		String rootUrl = request.getContextPath() + "/"+ ConfigUtil.get("fileSavePath") +"/";

		// 图片扩展名
		String[] fileTypes = new String[] { "gif", "jpg" };
	...
	@ResponseBody
	public Map<String, Object> uploadfile(HttpServletResponse response, HttpServletRequest request, HttpSession session) {
		Map<String, Object> m = new HashMap<String, Object>();
		m.put("error", 1);
		m.put("message", "上传失败！");
		// 文件保存目录路径
		String savePath = session.getServletContext().getRealPath("/") +ConfigUtil.get("fileSavePath") +"/";

		// 文件保存目录URL
		String saveUrl = request.getContextPath() + "/"+ConfigUtil.get("fileSavePath")+"/";
